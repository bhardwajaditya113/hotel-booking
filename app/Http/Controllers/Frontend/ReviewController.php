...
class ReviewController extends Controller
{
    ...
    public function store(Request $request, $bookingId)
    {
        ...
        $r = (int) $validated['overall_rating'];

        // Create review (columns match 2026_01_06_create_reviews_table)
        $review = Review::create([
            'user_id' => Auth::id(),
            'room_id' => $booking->rooms_id,
            'booking_id' => $booking->id,
            'rating_overall' => $r,
            'rating_cleanliness' => (int) ($validated['cleanliness_rating'] ?? $r),
            'rating_location' => (int) ($validated['location_rating'] ?? $r),
            'rating_value' => (int) ($validated['value_rating'] ?? $r),
            'rating_service' => (int) ($validated['service_rating'] ?? $r),
            'rating_amenities' => (int) ($validated['amenities_rating'] ?? $r),
            'rating_comfort' => (int) ($validated['comfort_rating'] ?? $r),
    ...
    public function getReviews(Request $request, $roomId)
    {
        ...
        switch ($request->sort) {
            ...
            case 'highest':
                ...
                break;
            case 'lowest':
                $query->orderBy('rating_overall', 'asc');
                break;
            case 'helpful':
    ...
    public function report(Request $request, $reviewId)
    {
        ...
        if ($reportCount >= 3) {
            $review->update(['is_approved' => false, 'is_featured' => false]);
        }
