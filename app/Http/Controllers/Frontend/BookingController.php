    ...
    public function Checkout(){
        ...
        if (Session::has('book_date')) {
           ...
           if (isset($book_data['property_id'])) {
               $property = \App\Models\Property::find($book_data['property_id']);
    ...
    public function CheckoutStore(Request $request){
        ...
        $this->validate($request,[
         ...
         if ($request->payment_method == 'Stripe') {
            ...
            // For Razorpay, we first create booking then redirect to payment
            $data = new Booking();
        ...
        foreach ($user as $admin) {
            DB::table('notifications')->insert([
                ...
                'user_id' => $admin->id,
                'data' => json_encode(['message' => 'New Booking Added by ' . $request->name]),
                'read_at' => null,
                'created_at' => now(),
                'updated_at' => now(),
            ]);
