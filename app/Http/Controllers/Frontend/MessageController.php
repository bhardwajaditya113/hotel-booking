...
class MessageController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $conversations = Conversation::where(function ($q) use ($user) {
            ...
            ->when($request->boolean('archived'), function ($q) use ($user) {
                $q->where(function ($q2) use ($user) {
                    $q2->where(function ($q3) use ($user) {
                        $q3->where('user1_id', $user->id)->where('user1_archived', true);
                    })->orWhere(function ($q3) use ($user) {
                        $q3->where('user2_id', $user->id)->where('user2_archived', true);
                    });
                });
            }, function ($q) use ($user) {
                $q->where(function ($q2) use ($user) {
                    $q2->where(function ($q3) use ($user) {
                        $q3->where('user1_id', $user->id)->where('user1_archived', false);
                    })->orWhere(function ($q3) use ($user) {
                        $q3->where('user2_id', $user->id)->where('user2_archived', false);
                    });
                });
            })
